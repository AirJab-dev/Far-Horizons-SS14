...
";
        ...
        public async Task TestAllRestocksAreAvailableToBuy()
        {
            ...
            await server.WaitAssertion(() =>
            {
                ...
                {
                    // If this prototype's ProductEntity is a restock, remove it from the set of restocks to check.
                    if (restockEntities.Contains(listing.ProductEntity.ToString()))
                    {
                        restockEntities.Remove(listing.ProductEntity.ToString());
                    }
                }

                // Don't check ones we explicitly marked as not obtainable (e.g. for mappers or admins)
                restockEntities.RemoveWhere(x => _ignoredPrototypes.Contains(x));
                // Starlight end
